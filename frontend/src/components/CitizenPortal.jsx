...
function CitizenPortal() {
  ...
  const [manualAddress, setManualAddress] = useState(""); // optional user-typed address
  ...
  // Save GPS location
  const [location, setLocation] = useState(null); // GeoJSON { type: "Point", coordinates: [lng, lat] }
  const [locError, setLocError] = useState("");
  const [humanAddress, setHumanAddress] = useState(""); // Converted address
  ...
  const getAddressFromCoords = async (lat, lng) => {
    try {
      ...
      if (data && data.display_name) {
        ...
        setHumanAddress("Address not found");
      }
    } catch (error) {
  ...
  const getLiveLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        setLocError("Unable to fetch location: " + err.message);
      }
    );
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!reportType || !title || !description || (!location && !manualAddress)) {
      return;
    }
    ...
    try {
      await api.post("/citizen/submit-issue", formData, {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-white to-green-50">
        <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full text-center">
          ...
          <p className="text-gray-600 mb-6">
            You must be signed in as a citizen to report issues or view your
            reports.
          </p>
              ...
              userReports.map((report) => (
                ...
                >
                  ...
                  {(report.imageUrl && report.imageUrl !== "null") ? (
                    ...
                    <div className="mt-3 sm:mt-0 sm:ml-4 w-full sm:w-40 h-40 flex items-center justify-center bg-gray-100 rounded-lg border text-gray-400">
                      No image
                    </div>
                  )}
                </div>
